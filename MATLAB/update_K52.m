...
%% Processing data for serial communications
    ...
    % In serial communication, the computer will communicate data in a
    % stream of characters out of its USB port, to our microcontroller. We
    % need to decide on a format that this stream of characters will be in.
    % It is desirable to be consident, and needs to have identifiers that will
    % let the microcontroller easily decide which characters (numbers)
    % belong to which value (which sunsensor or magnetic angle).
        %clc
        global sundata
        global solint
        global it
     ...
     % Two functions make variables out of a line of the two text files.
    ...
    % It turns out that we want AzEl data for each sun sensor. This
    % involved choosing a standard reference for the coordinate system axes
    % for each individual sensor. Regardless of placement or orientation,
    % the +X axis of each sensor's body axis was chosen as zero for
    ...
    % Convert sunsensor unit vectors into azimuth, elevation, and range
     ...
     % While it may seem undesirable to do so, each matrix element is called
     % out as its own variable for string operations. Not doing this
     % results in scientific notation in the packet, ruining the format.
...
%% Constrain for solar intensity (is it in the Earth's shadow?), and cone width of sensor (what it can actually see)
    ...
    % Since the scenario is all second-by-second we do not have to have
    % some fancy algorithm that analyzes the vector components to see if
    % the sun angle is out of the cone widths of the sensors, simply, the
...
%% Format the AzEl numbers into consistent strings
    ...
    % negative signs mess with formatSpec1, and so the values are made as
...
%         end
    ...
    % Always be in form of 4 decimals and 1 integer. The '0' pads
    % the front with any leading zeros necessary to maintain format. The 6
    % is the field width, decimal is decimal, 4 is decimal places, and f is
    % fixed number.
